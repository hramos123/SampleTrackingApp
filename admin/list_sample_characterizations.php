...
{
	$edit_query="DELETE FROM `sample_characterizations` WHERE `id`='$_REQUEST[delid]'";
	if(@mysql_query($edit_query))
	{
		header("location:list_sample_characterizations.php?msg=ais");
	}
	else
	{
		header("location:list_sample_characterizations.php?msg=aif");
	}
}
...
$del_id = $checkbox[$i];
$sql = "DELETE FROM `sample_characterizations` WHERE id='$del_id'";
mysql_query($sql); 
}
header("location:list_sample_characterizations.php?msg=ais");
}
	...
	<?php include('includes/admin_header.php');?>
		<div class="admin_area">
			...
			<div class="admin_module_area">
				...
				<div class="admin_title_text">Sample Characterizations List</div></div><!--admin_title ends-->
				...
				<table width="800">
					...
					<tr>
						<th width="5%"><input type="checkbox" onselect="toggleCheckboxes(true)" /></th>
						<th width="5%">ID</th>
						<th width="30%">Characterization Name</th>
						<th width="50%">Characterization Description</th>
						<th width="10%">Action</th>
					</tr>
					<?php
					
					$list_query = mysql_query("SELECT * FROM `sample_characterizations` ORDER BY `id` DESC");
					...
					<tr>
						...
						<td>&nbsp;&nbsp;<img src="images/b_edit.png" title="edit" style="cursor:pointer;" onclick="window.location='add_sample_characterization.php?id=<?php echo $list['id'];?>'" />&nbsp;&nbsp;&nbsp;&nbsp;
						<a href="list_sample_characterizations.php?delid=<?php echo $list['id'];?>"><img src="images/b_drop.png" title="Delete" style="cursor:pointer;" onclick="return confirm('are you sure want to delete this sample characterization!');" /></a></td>
					</tr>
					<?php } ?>
				</table>
				</form>
