...
{	
	...
	$sql="INSERT INTO `assays` values('','$title')";
	if(@mysql_query($sql))
	{
		header("location:add_assays.php?msg=ais");
	}
	else
	{
		header("location:add_assays.php?msg=aif");
	}
}
...
{	
	...
	$sql="UPDATE `assays` SET `name`='$title' WHERE `id`='$_REQUEST[id]'";
	if(@mysql_query($sql))
	{
		header("location:add_assays.php?msg=ais&id=$_REQUEST[id]");
	}
	else
	{
	...
	<?php include('includes/admin_header.php');?>
		<div class="admin_area">
			...
			<div class="admin_module_area">
				<div class="admin_title"><div class="admin_title_img"><img src="images/icon_content.gif" style="height:30px; vertical-align:middle;" /></div><div class="admin_title_text">Add Assays</div></div><!--admin_title ends-->
				<table width="800">
					<?php @$fetch = mysql_fetch_array(mysql_query("SELECT * FROM `assays` WHERE `id`='$_REQUEST[id]'"));?>
					<form method="post">
					<?php if($_GET['msg']=='aif'){?>
					<tr>
