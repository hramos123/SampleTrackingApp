...
{
	$edit_query="DELETE FROM `experiments` WHERE `id`='$_REQUEST[delid]'";
	if(@mysql_query($edit_query))
	{
		header("location:list_experiment.php?msg=ais");
	}
	else
	{
		header("location:list_experiment.php?msg=aif");
	}
}
...
$del_id = $checkbox[$i];
$sql = "DELETE FROM `experiments` WHERE id='$del_id'";
mysql_query($sql); 
}
header("location:list_experiment.php?msg=ais");
}
	...
	<?php include('includes/admin_header.php');?>
		<div class="admin_area">
			...
			<div class="admin_module_area">
				...
				<div class="admin_title_text">Experiment List</div></div><!--admin_title ends-->
				...
				<table width="800">
					...
					<tr>
						...
						<th width="5%">ID</th>
						<th width="40%">Experiment Name</th>
					...
					$list_query = mysql_query("SELECT id, name, (select name from projects p where p.id=e.project_id) as project_name FROM experiments e ORDER BY id DESC") or die(mysql_error());
					...
					<tr>
						...
						<td>&nbsp;&nbsp;<img src="images/b_edit.png" title="edit" style="cursor:pointer;" onclick="window.location='add_experiment.php?id=<?php echo $list['id'];?>'" />&nbsp;&nbsp;&nbsp;&nbsp;
						<a href="list_experiment.php?delid=<?php echo $list['id'];?>"><img src="images/b_drop.png" title="Delete" style="cursor:pointer;" onclick="return confirm('are you sure want to delete this experiment!');" /></a></td>
					</tr>
					<?php } ?>
				</table>
				</form>
